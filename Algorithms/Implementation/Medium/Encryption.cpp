...
string encryption(string s){
   string str;
    for(int i=0;i<s.length();i++){
        //Making new string by omiting every space.
        if(s[i]!=' '){
            str.push_back(s[i]);
        }
    ...
    if(rows*columns<str.length()){
        //If the 2D matrix we going to make, doesn't have the capacity to hold the string, 
        //then we increase number of rows by 1.
        rows++;
    } 
  ...
  for(int i=0;i<rows;i++){
      for(int j=0;j<columns;j++){  
          if(k>str.length()){
              //When string 'str' length is smaller than the (rows X column), we fill the empty
              arr[i][j]='\0'; 
          }
  ...
  for(int i=0;i<columns;i++){
      ...
      for(int j=0;j<rows;j++){
          if(arr[j][i]=='\0'){
            //We don't want the garbage values, we filled before.
            continue;
          }  
